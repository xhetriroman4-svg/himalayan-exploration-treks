  ...
  return (
    <main className="min-h-screen bg-transparent text-[#f0f4f8] overflow-x-hidden relative z-0">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {TOURS.slice(0, 6).map((tour) => {
              ...
              return (
              <div key={tour.title} className="card-premium holo-sheen gradient-border-flow overflow-hidden reveal-fade-up group">
                {/* Image with overlay */}
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-white/10">
                    <div>
                      ...
                      <div className="font-cinematic text-2xl font-bold text-golden-shimmer price-pulse">${tour.price.toLocaleString()}</div>
                    </div>
      ...
      <section className="py-16 px-4 section-wash-gold">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-8 reveal">
            <span className="pill-cinematic mb-4">🎥 Video Stories</span>
            <h2 className="font-cinematic text-3xl sm:text-4xl font-bold mt-4 mb-2 text-readable-strong">
              Hear From <span className="text-golden-shimmer italic">Our Trekkers</span>
            </h2>
            <div className="divider-golden" />
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 reveal">
            {[
              { name: 'Sarah', trip: 'EBC Trek', image: 'https://sfile.chatglm.cn/images-ppt/931b93f13c50.jpg' },
              { name: 'Marcus', trip: 'Annapurna', image: 'https://sfile.chatglm.cn/images-ppt/beb25e4341d0.jpg' },
              { name: 'Priya', trip: 'Mustang', image: 'https://sfile.chatglm.cn/images-ppt/eccfdc1f7215.jpg' },
              { name: 'James', trip: 'Manaslu', image: 'https://sfile.chatglm.cn/images-ppt/eddc45d8a60a.jpg' },
            ].map((v) => (
              <div key={v.name} className="testimonial-video-card" onClick={() => setVideoModalOpen(true)}>
                <img src={v.image} alt={v.name} loading="lazy" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
                <div className="testimonial-video-play">
                  <svg className="w-6 h-6 text-black ml-0.5" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
                </div>
                <div className="absolute bottom-3 left-3 right-3">
                  <div className="text-sm font-display font-bold text-white text-readable-strong">{v.name}</div>
                  <div className="text-xs text-white/80">{v.trip}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="fab-container">
        <a href="https://wa.me/9779841023371" target="_blank" rel="noopener noreferrer" className="fab fab-whatsapp" title="WhatsApp us" aria-label="WhatsApp">
        ...
        <a href="tel:+9779841023371" className="fab fab-call" title="Call us" aria-label="Call">
        ...
        <button
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className={`fab fab-top ${backToTopVisible ? 'visible' : ''}`}
          title="Back to top"
          aria-label="Back to top"
        >
          <svg className="w-5 h-5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 19V5M5 12l7-7 7 7"/></svg>
        </button>
      </div>

      {/* ═══════════ CUSTOM MOUNTAIN CURSOR ═══════════ */}
      <div
        className={`cursor-mountain ${cursorHovering ? 'hovering' : ''}`}
        style={{ left: `${cursorPos.x}px`, top: `${cursorPos.y}px` }}
        aria-hidden
      >
        <svg viewBox="0 0 16 16" fill="currentColor" style={{ color: '#d4a853' }}>
          <path d="M8 1L1 15h14L8 1z" opacity="0.9"/>
          <path d="M8 1L11 7L8 6L5 7L8 1z" fill="#fff" opacity="0.4"/>
        </svg>
      </div>
